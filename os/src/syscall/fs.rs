...
pub fn sys_write(fd: usize, buf: *const u8, len: usize) -> isize {
    ...
    if let Some(file) = &inner.fd_table[fd] {
        ...
        file.write(UserBuffer::new(
            translated_byte_buffer(token, buf, len, false).unwrap(),
        )) as isize
    } else {
        -1
    }
...
pub fn sys_read(fd: usize, buf: *const u8, len: usize) -> isize {
    ...
    if let Some(file) = &inner.fd_table[fd] {
        ...
        file.read(UserBuffer::new(
            translated_byte_buffer(token, buf, len, true).unwrap(),
        )) as isize
    } else {
        -1
    }
...
pub fn sys_fstat(_fd: usize, _st: *mut Stat) -> isize {
    ...
    if let Some(file) = &inner.fd_table[_fd] {
        let file = file.clone();
        drop(inner);
        if let Some(stat) = file.file_stat() {
            if copy_to_user(token, _st, &stat) {
                0
            } else {
                -1
            }
